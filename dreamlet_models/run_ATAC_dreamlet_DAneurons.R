...
df$experiment <- str_extract(df$sample, "first_experiment|replicate_experiment|outgroup_experiment|rotenone_experiment")
df$indiv <- str_extract(df$sample, paste(unique(sce1$indiv), collapse = '|'))

# Calculate downsampling
human_samples <- df %>% filter(species == "human")
chimp_samples <- df %>% filter(species == "chimp")
macaque_samples <- df %>% filter(species == "macaque")
