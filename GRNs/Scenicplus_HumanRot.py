...
import os
work_dir = '/wynton/group/pollen/jding/Sara/data/scenicplus'
tmp_dir = 'wynton/scratch/jding/'


# ## Loading preprocessed @Scanpy_D40Human.ipynb
...
directory='/wynton/group/pollen/sara_bam_d80_221216'
...
namesOfInterest=['D80_M_0-6_CNTRL_Human', 'D80_M_0-6_24H_Human', 'D80_M_0-8_24H_Human',
                 'D80_M_0-6_72H_Human', 'D80_M_0-8_72H_Human','D80_M_0-8_CNTRL_Human'
                ]
...
adatas=[]   #add read multiseq barcodes csv here

for f in fileList:
    print(f)
    adata = sc.read_10x_h5(os.path.join(directory,f,'outs','filtered_feature_bc_matrix.h5'))
    #multiseq_calls=pd.read_csv(os.path.join(directory,f,'outs','multiseq_calls.csv'), index_col=0)
    #multiseq_calls=multiseq_calls.loc[multiseq_calls.index.isin(adata.obs.index),:]
    #adata.obs['multiseq_calls']='nan'
    #adata.obs.loc[multiseq_calls.index,'multiseq_calls']=multiseq_calls['assignment']
    #adata.obs['Singlets_doublets']='nan'
    #adata.obs.loc[multiseq_calls.index,'Singlets_doublets']=multiseq_calls['demux_type']
...
#import scanpy as sc
#ad = sc.read('/wynton/group/pollen/sara/20240104_GEX_D80_rot_challenge.h5ad')
#sc.pl.umap(ad, color = 'supervised_name', legend_loc = 'on data')

import scanpy as sc
#ad = sc.read('/wynton/group/pollen/jding/Sara/data/scRNA/20240303_GEX_D80_rot_challenge_rPCA_DA_subset.h5ad')
#ad = sc.read('/wynton/group/pollen/jding/Sara/data/scRNA/20240305_GEX_D80_rot_challenge_rPCA_DA_subset.h5ad')
ad = sc.read('/wynton/group/pollen/jding/Sara/20241011_GEX_D80_rot_challenge_rPCA_DA_subset.h5ad')
ad.obs['barcode'] = [x.split("_")[-1] for x in ad.obs_names]
ad.obs.index = ad.obs['barcode'].astype(str) +'-'+ad.obs['orig.ident'].astype(str)
sc.pl.umap(ad, color = 'supervised_name', legend_loc = 'on data')


#ad = sc.read('/wynton/group/pollen/jding/Sara/data/scRNA/20240328_GEX_D80_rot_challenge_DA_subset_human.h5ad')
#sc.pl.umap(ad, color = 'supervised_name', legend_loc = 'on data')

# In[13]:

adata = adata[adata.obs.index.isin(ad.obs.index)]
ad = ad[adata.obs.index]
print(ad)

# In[14]:


for x in ad.obs.columns.tolist():
    adata.obs[x]=ad.obs[x]
...
fragments_dict= {'D80_M_0-6_24H_Human': '/wynton/group/pollen/sara_bam_d80_221216/D80_M_0-6_24H_Human/outs/atac_fragments.tsv.gz',
 'D80_M_0-6_72H_Human': '/wynton/group/pollen/sara_bam_d80_221216/D80_M_0-6_72H_Human/outs/atac_fragments.tsv.gz',
 'D80_M_0-6_CNTRL_Human': '/wynton/group/pollen/sara_bam_d80_221216/D80_M_0-6_CNTRL_Human/outs/atac_fragments.tsv.gz',
 'D80_M_0-8_24H_Human': '/wynton/group/pollen/sara_bam_d80_221216/D80_M_0-8_24H_Human/outs/atac_fragments.tsv.gz',
 'D80_M_0-8_72H_Human': '/wynton/group/pollen/sara_bam_d80_221216/D80_M_0-8_72H_Human/outs/atac_fragments.tsv.gz',
...
genome = '/wynton/group/pollen/genomes/cellranger-arc/refdata-cellranger-arc-GRCh38-2020-A-2.0.0'
...
bw_paths, bed_paths = export_pseudobulk(input_data = cell_data,
                 variable = 'supervised_name',                                                                     # variable by which to generate pseubulk profiles, in this case we want pseudobulks per celltype
                 sample_id_col = 'batch',
...
path_to_regions={'D80_M_0-6_24H_Human': '/wynton/group/pollen/jding/Sara/data/scenicplus/scATAC/consensus_peak_calling/consensus_regions.bed',
 'D80_M_0-6_72H_Human': '/wynton/group/pollen/jding/Sara/data/scenicplus/scATAC/consensus_peak_calling/consensus_regions.bed',
 'D80_M_0-6_CNTRL_Human': '/wynton/group/pollen/jding/Sara/data/scenicplus/scATAC/consensus_peak_calling/consensus_regions.bed',
 'D80_M_0-8_24H_Human': '/wynton/group/pollen/jding/Sara/data/scenicplus/scATAC/consensus_peak_calling/consensus_regions.bed',
 'D80_M_0-8_72H_Human': '/wynton/group/pollen/jding/Sara/data/scenicplus/scATAC/consensus_peak_calling/consensus_regions.bed',
 'D80_M_0-8_CNTRL_Human': '/wynton/group/pollen/jding/Sara/data/scenicplus/scATAC/consensus_peak_calling/consensus_regions.bed'}
...
qc_filters={'D80_M_0-6_24H_Human': {'Log_unique_nr_frag': [3, None],
  'FRIP': [0.2, None],
  'TSS_enrichment': [4, None],
  'Dupl_rate': [None, None]},
 'D80_M_0-6_72H_Human': {'Log_unique_nr_frag': [3, None],
  'FRIP': [0.2, None],
  'TSS_enrichment': [4, None],
  'Dupl_rate': [None, None]},
 'D80_M_0-6_CNTRL_Human': {'Log_unique_nr_frag': [3, None],
  ...
  'TSS_enrichment': [4, None],
  'Dupl_rate': [None, None]},
 'D80_M_0-8_24H_Human': {'Log_unique_nr_frag': [3, None],
  'FRIP': [0.2, None],
  'TSS_enrichment': [4, None],
  'Dupl_rate': [None, None]},
 'D80_M_0-8_72H_Human': {'Log_unique_nr_frag': [3, None],
  'FRIP': [0.2, None],
  'TSS_enrichment': [4, None],
  'Dupl_rate': [None, None]},
...
cell_data['barcode'] = cell_data.index.astype(str) +'___'+ cell_data['batch'].astype(str)
...
print(cistopic_obj.subset(index, copy=True))
...
for key in list(markers_dict):
    if markers_dict[key].empty:
        del markers_dict[key]
markers_dict
...
fpath = "/wynton/group/pollen/jding/Sara/data/scenicplus/scATAC/"
...
#rankings_db = os.path.join(motif_fpath, 'hg38_screen_v10_clust.regions_vs_motifs.rankings.feather')
#scores_db =  os.path.join(motif_fpath, 'hg38_screen_v10_clust.regions_vs_motifs.scores.feather')
...
dataset = pbm.Dataset(name=species+'_gene_ensembl',  host=biomart_host)
annot = dataset.query(attributes=['chromosome_name', 'transcription_start_site', 'strand', 'external_gene_name', 'transcript_biotype'])
annot.columns = ['Chromosome', 'Start', 'Strand', 'Gene', 'Transcript_type']
...
menr['DEM_topics_otsu_All'].DEM_results()
...
adata.obs.index = adata.obs['barcode'].astype(str) +'-1___'+ adata.obs['batch'].astype(str)
adata.obs
...
#remove low count clusters: MB_HB_neurons_LHX1, FB_MB_glutamatergic_neurons, Mixed_neurons_(macaque)                   
#index=scplus_obj.metadata_cell[scplus_obj.metadata_cell.ACC_supervised_name.isin([
#       'Glutamatergic_neurons', 'Dopaminergic_neurons',
#       'Dopaminergic_neurons_stressed', 'Progenitor_cells',
#      'Oculomotor_neurons', 'Glutamatergic_neurons_stressed',
#       'Stressed_neurons', 'MB_HB_glutamatergic_neurons',
#       'MB_HB_glutamatergic_neurons_stressed', 
#       'MB_GABAergic_neurons'])].index.tolist()
#scplus_obj=scplus_obj.subset(cells=index,return_copy=True)
...
#annot.to_csv('/wynton/home/pollenlab/jding/scenicplus/hg38_annot_scenicplus.csv')
#annot = pd.read_csv('/wynton/home/pollenlab/jding/scenicplus/hg38_annot_scenicplus.csv')
...
plot_metadata_given_ax(
    ...
    ax = axs[1],
    reduction_name = 'eRegulons_UMAP',
    variable = 'GEX_condition', #note the GEX_ prefix, this metadata originated from the gene expression metadata (on which we did the cell type annotation before)
    #color_dictionary={'GEX_celltype': color_dict}
)
fig.tight_layout()
...
plot_eRegulon(
    scplus_obj = scplus_obj,
    reduction_name = 'eRegulons_UMAP',
    selected_regulons = ['NFE2L2_+', 'NRF1_+','FOS_+', 'JUN_+'],
    scale = True,
    auc_key = 'eRegulon_AUC_filtered')
...
plot_eRegulon(
    scplus_obj = scplus_obj,
    reduction_name = 'eRegulons_UMAP',
    selected_regulons = ['ZBTB20_+', 'PBX3_+','KLF12_+', 'YBX1_+'],
    scale = True,
    auc_key = 'eRegulon_AUC_filtered')
...
#plt.save('/wynton/home/pollenlab/jding/Sara/scenicplus/figures/scenicplus/HumanRot_selection.pdf')
plt.show()
...
heatmap_dotplot(
        ...
        index_order = ['Progenitor_cells',  'Glutamatergic_neurons','MB_HB_glutamatergic_neurons',
       'MB_GABAergic_neurons','Oculomotor_neurons','Dopaminergic_neurons',
       'Stressed_neurons', 'MB_HB_glutamatergic_neurons_stressed',
       'Glutamatergic_neurons_stressed', 'Dopaminergic_neurons_stressed',
        ],
        figsize = (5, 8),
        orientation = 'vertical',
        save='/wynton/home/pollenlab/jding/Sara/scenicplus/figures/scenicplus/HumanRot_heatmap.pdf')
...
plot_rss(scplus_obj, 'GEX_supervised_name_filtered', num_columns=4, top_n=20, figsize = (20, 10),
        save='/wynton/home/pollenlab/jding/Sara/scenicplus/figures/scenicplus/HumanRot_rss.pdf')
