...
base_folder = "/media/jenelle/4TB_disk/Dropbox/Analysis/Dreamlet_R_4.4.0/Midbrain/Ancestral_genome/"
folder = paste0(base_folder,"D16/V9/"); dir.create(folder) #folder to save results from this version of the model
...
sce_orig = readRDS(paste0(base_folder, "sce_new_16.rds"))
sce1 = sce_orig
...
#Fix pool type
colData(sce)$pool_type[sce$species == 'human' & sce$pool_type == 'Intraspecies'] = 'Intraspecies_human' 
colData(sce)$pool_type[sce$species == 'chimp' & sce$pool_type == 'Intraspecies'] = 'Intraspecies_chimp' 
colData(sce)$pool_type[sce$species == 'macaque' & sce$experiment == 'outgroup_experiment'] = 'Intraspecies_macaque' 
colData(sce)$pool_type[sce$species == 'orangutan' & sce$experiment == 'outgroup_experiment'] = 'Intraspecies_orangutan' 
...
sce$sample <- paste(sce$species, sce$experiment, sce$indiv, sce$sex, sce$pool_type, sce$lane, sep = "_")  #sample column determines which variables to use - needs to match equation
...
form = ~ species  + (1|experiment) + (1|indiv) + (1|sex) + (1|lane) + (1|pool_type)
...
vp_form = ~  (1|species) + (1|experiment) + (1|indiv)+ (1|sex) + (1|lane) + (1|pool_type)
...
# Dreamlet analysis----
d_form = ~ 0 + species + (1|experiment) + (1|indiv)+ (1|sex) + (1|lane) + (1|pool_type)
