...
folder = paste0(base_folder,"D40_D100_D80/V29/"); dir.create(folder) #folder to save results from this version of the model
...
include_celltypes = c("DA_STN_neurons_immature", "DA_neurons")
cells_to_keep <- colData(sce2)$cell_type %in% include_celltypes; sce <- sce2[, cells_to_keep]
...
pseudotime_df <- read.csv(paste0(folder, 'merged.DA.monocle3.csv')) %>%
  rename(barcode = X)
new_cols <- c('monocle3_pseudotime')
new_meta <- left_join(meta_df, pseudotime_df %>% dplyr::select(all_of(c("barcode",new_cols))), by = "barcode") %>%
  rename(pseudotime = monocle3_pseudotime)
rownames(new_meta) <- new_meta$barcode
if(all(rownames(new_meta) != colnames(sce))){print('Warning - cells not in the same order!')}
colData(sce) <- S4Vectors::DataFrame(new_meta)
...
form = ~ species  + (1|experiment) + pseudotime  + (1|indiv) + (1|sex)
...
ggsave(paste0(figure_folder, 'voom_plot.png'))

#Variance partitioning analysis----
vp_form = ~  (1|species) + (1|experiment) + pseudotime + (1|indiv)+ (1|sex)
...
ggsave(paste0(figure_folder, 'vp_plot.png'), height=7, width=11, units = "in")

# Dreamlet analysis----
d_form = ~ 0 + species + (1|experiment) + pseudotime + (1|indiv)+ (1|sex)
